...
//
	...
	static BsonValue bookHall(@PathVariable String startDate, @PathVariable String endDate, @PathVariable String capacity, @PathVariable String hallName) {
		// TODO Auto-generated method stub
...
//				LocalDateTime sd = LocalDateTime.parse(startDate, DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss"));;
//			     LocalDateTime ed = LocalDateTime.parse(endDate, DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss"));;
			Document doc = new Document().append("startDate",sd).append("endDate",ed).append("capacity",capacity).append("hallName",hallName);
...
//		 System.out.println(ress);
	...
	//without path variable
	 void  bookHallwopv( LocalDateTime startDate, LocalDateTime endDate, String capacity, String hallName) {
		// TODO Auto-generated method stub
...
//	 				multi_hall ob2 = new multi_hall(det[i+1], collection);
	...
	@PostMapping("/bookmultipleweecutor/")
	private static void bookmultipleweexec() throws ParseException, FileNotFoundException {
...
//	 			jsonObject
	 	    ...
	 	    for (int i = 0; i < 50; i++) { 
	 	      service.submit(new multi_hall(det[i],collection)); 
	 	    } 
	...
	@DeleteMapping("/del/")
	void del() {
		Bson filter = Filters.ne("hallName","Hall-G");
//		Bson filter = Filters.and(Filters.and(Filters.gte("startDate",startDate ), Filters.lte("endDate",endDate)),Filters.eq("hallName",hallName));
